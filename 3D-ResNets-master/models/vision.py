...
savepath = '/home/zhangyong/origine/picture'
...
#  可视化特征图
def show_feature_map(feature_map_t):
    print(feature_map_t.shape)
    # feature_map_t = feature_map_t.permute(1, 0, 2, 3)

    # feature_map=torch.Size([1, 64, 55, 55]),feature_map[0].shape=torch.Size([64, 55, 55])
    # feature_map[2].shape     out of bounds [1, 16, 64, 55, 55]
    time_num = len(feature_map_t[0])
    for i in range(1, time_num+1):
        feature_map = feature_map_t[i]
        ...
        upsample = torch.nn.UpsamplingBilinear2d(size=(112, 112))  # 这里进行调整大小
        ...
        # row_num = feature_map_num / 8
        plt.figure()
        for index in range(1, feature_map_num + 1):  # 通过遍历的方式，将64个通道的tensor拿出

            plt.subplot(row_num, row_num, index)
            plt.imshow(feature_map[index - 1].cpu().detach(), cmap='gray')
            plt.show()# feature_map[0].shape=torch.Size([55, 55])
            # plt.imshow(transforms.ToPILImage()(feature_map[index - 1])) # feature_map[0].shape=torch.Size([55, 55])
            plt.axis('off')
            plt.savefig('1.jpg')
            # plt.savefig('feature_map_save//' + 'time_' +str(i) + '_' + str(index) + ".png", feature_map[index - 1])
        plt.show()
